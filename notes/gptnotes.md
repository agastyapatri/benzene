#   Notes on Transformers and GPT-2
...
##  GPT-2
GPT-2 is a very large, decoder-only-transformer based model trained on a massive dataset. It was trained on the WebText Dataset scraped by OpenAI.

GPT-2 was trained in a couple of configurations vis-a-vis model size: 
-   Small:  
-   Medium 
-   Large

| Configuration | Parameters (M) | Embedding Dimension |Context Vector  |Number of Transformer Blocks  |Number of Attention Heads |
|:------------- |:--------------:| :--------------:    |:--------------:|:--------------:              |:--------------:          |
| Small         | 124            | 768                 |1024            |12                            |12                        |
| Medium        | 355            | 1024                |1024            |24                            |16                        |
| Large         | 744            | 1280                |1024            |36                            |20                        |
| Extra Large   | 1558           | 1600                |1024            |?                             |25                        |

GPT-2 is built by stacking transformer decoder blocks, as opposed to BERT which uses transformer-encoder blocks. GPT-2 outputs one token at a time in an **autoregressive** fashion, meaning that each token produced is added to the sequence of inputs which produced it as the input for the next iteration of the model input.

BERT is not autoregressive; BERT has the ability to incorporate the context on both sides of a word to gain better results. 

### Looking inside GPT-2
-   "generating unconditional samples": inputting a start token to the language model and letting it iteratively generate as many tokens as it wants; "letting it ramble"
-   "interactive conditional samples:" Giving the model a prompt or asking it to speak on a specific topic.





##  FIGURE OUT HOW TO STRUCTURE THE NOTES BELOW

GPT-2 small is built upon a stack of 12 identical transformer decoder blocks. Unlike the original "Attention is all you need" transformer architecture, which uses both and encoder and a decoder, GPT-2 uses only the decoder stack. 
